...
import { createParentPortalToken } from '@/lib/parent-portal';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const student = await prisma.student.findUnique({
      where: { id: studentId },
      select: { id: true, status: true, parentPhone: true },
    });

    if (!student || student.status !== 'ACTIVE' || !student.parentPhone) {
    ...
    return NextResponse.json(
      {
        token,
        url: `${baseUrl.replace(/\/$/, '')}/parent-portal/${token}`,
      },
